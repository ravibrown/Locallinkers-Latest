...
{
    public class ClsProductImages : EntityContext
    {
        #region "Properties"
        //Id
        private Int64 _Id = 0;
        ...
        public void Add()
        {
            tbl_ProductImages obj = _db.tbl_ProductImages.FirstOrDefault(a => a.Id == Id);
            if (obj == null)
            {
                obj = new tbl_ProductImages();
                SetObjects(obj);
                _db.tbl_ProductImages.Add(obj);
                _db.SaveChanges();
                Id = obj.Id;
                DataRecieved = true;
            }
        ...
        public void Delete()
        {
            ...
            if (obj != null)
            {
                _db.tbl_ProductImages.Remove(obj);
                _db.SaveChanges();
                DataRecieved = true;
            }
        }
        ...
        {
            List<tbl_ProductImages> obj = _db.tbl_ProductImages.Where(a => a.ProductId == ProductId).ToList();
            if (obj != null && obj.Count>0)
            {
                foreach (var item in obj)
                {
                    _db.tbl_ProductImages.Remove(item);
                    _db.SaveChanges();
                    DataRecieved = true;
                }
            }
        ...
        public void SetObjects(tbl_ProductImages obj)
        {
        ...
        public List<ClsProductImages> GetAll()
        {
            List<ClsProductImages> lst = new List<ClsProductImages>();
            lst = (from r in _db.tbl_ProductImages
                   where ((Id != 0 ? r.Id == Id : Id == 0) &&
                   (IsDeleted ? r.IsDeleted == true : r.IsDeleted == false) &&
                   (ProductId != 0 ? r.ProductId == ProductId : ProductId == 0))

                   select new ClsProductImages
                   {
                       Id = r.Id,
            ...
            lst = lst.Select((r, index) => new ClsProductImages
            {
                Id = r.Id,
                ProductId = r.ProductId,
                Image = r.Image,
        ...
        {
            tbl_ProductImages obj = _db.tbl_ProductImages.FirstOrDefault(r => (Id != 0 ? r.Id == Id : Id == 0) &&
                   (IsDeleted ? r.IsDeleted == true : r.IsDeleted == false) &&
                   (ProductId != 0 ? r.ProductId == ProductId : ProductId == 0));
            if (obj != null)
            {
