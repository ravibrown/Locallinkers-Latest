...
{
    public class ClsTemplateSlider : EntityContext
    {
        #region "Properties"
        //Id
        private Int64 _Id = 0;
        ...
        public void Add()
        {
            tbl_TemplateSliders obj = _db.tbl_TemplateSliders.FirstOrDefault(a => a.Id == Id);
            if (obj == null)
            {
                obj = new tbl_TemplateSliders();
                SetObjects(obj);
                _db.tbl_TemplateSliders.Add(obj);
                _db.SaveChanges();
                Id = obj.Id;
                DataRecieved = true;
            }
        ...
        {
            List<tbl_TemplateSliders> obj = _db.tbl_TemplateSliders.Where(a => a.TemplateId == TemplateId).ToList();
        ...
        public void Delete()
        {
            ...
            if (obj != null)
            {
                _db.tbl_TemplateSliders.Remove(obj);
                _db.SaveChanges();
                DataRecieved = true;
            }
        }
        ...
        public void DeleteByTemplateId()
        {
            ...
            if (obj != null && obj.Count>0)
            {
                foreach (var item in obj)
                {
                    _db.tbl_TemplateSliders.Remove(item);
                    _db.SaveChanges();
                    DataRecieved = true;
                }
            }
        ...
        public void SetObjects(tbl_TemplateSliders obj)
        {
        ...
        public List<ClsTemplateSlider> GetAll()
        {
            List<ClsTemplateSlider> lst = new List<ClsTemplateSlider>();
            lst = (from r in _db.tbl_TemplateSliders
                   where ((Id != 0 ? r.Id == Id : Id == 0) &&
                   (IsDeleted ? r.IsDeleted == true : r.IsDeleted == false) &&
                   (TemplateId != 0 ? r.TemplateId == TemplateId : TemplateId == 0))

                   select new ClsTemplateSlider
                   {
                       Id = r.Id,
            ...
            lst = lst.Select((r, index) => new ClsTemplateSlider
            {
                Id = r.Id,
                TemplateId = r.TemplateId,
                Title = r.Title,
